 ...
 * @throws {Error} 401 - If the token is missing.
 * @throws {Error} 403 - If the token is invalid.
 */
/*******  494ce8ff-c5d0-4098-b9e3-f3375c319614  *******/    
...
app.get('/api/devices', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT * FROM data_device WHERE user_id = $1', 
            [req.user.userId]
        );
...
app.get('/api/sensor/latest/:deviceId', authenticateToken, async (req, res) => {
    try {
        const { deviceId } = req.params;
        // Check if device belongs to user
        const deviceCheck = await pool.query('SELECT id FROM data_device WHERE id = $1 AND user_id = $2', [deviceId, req.user.userId]);
